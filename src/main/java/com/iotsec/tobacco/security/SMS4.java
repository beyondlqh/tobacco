...
public class SMS4 {
	...
	private static final byte[] key = { 0x01, 0x23, 0x45, 0x67, (byte) 0x89,
	...
	public String en(String inS) {
		byte[] in = formatIn(inS).getBytes();
		int inLen = in.length;
		...
		return BinaryToHexString(out);
	}

	public String de(String inS) {
		byte[] in = HexStringToBinary(inS);
		int inLen = in.length;
		byte[] out = new byte[in.length];
		...
		if (new String(out).indexOf(new String(new byte[] { 0x01 })) == -1)
			return new String(out);
		return new String(out).substring(0,
				new String(out).indexOf(new String(new byte[] { 0x01 })));
	}
	...
	public String formatIn(String in) {
		...
		for (int i = in.length(); i < ((in.length() / 16) + 1) * 16; i++) {
